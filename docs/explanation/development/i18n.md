...
Find the English copy in the Spanish `django.po` file as a `msgid`, and add the corresponding Spanish translation as the `msgstr`. Again, run the helper script for formatting and `python manage.py compilemessages` to confirm the translation is rendered properly.
